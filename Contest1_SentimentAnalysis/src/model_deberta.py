...
def main():
    ...
    training_args_aspect = TrainingArguments(
        output_dir='./checkpoints/results_aspect_deberta',
        ...
        logging_dir='./logs_aspect_deberta',
        logging_steps=50,
        learning_rate=2e-5,
        save_total_limit=1,
        # bf16=True, # Disabled due to RuntimeError
        # fp16=True, # Disabled due to RuntimeError
        weight_decay=0.01,
        warmup_ratio=0.1
    )
    ...
    training_args_sent = TrainingArguments(
        output_dir='./checkpoints/results_sentiment_deberta',
        num_train_epochs=EPOCHS,
        per_device_train_batch_size=BATCH_SIZE,
        per_device_eval_batch_size=BATCH_SIZE,
        eval_strategy="epoch",
        ...
        logging_dir='./checkpoints/logs_sentiment_deberta',
        logging_steps=50,
        learning_rate=2e-5,
        save_total_limit=1,
        # bf16=True, # Disabled due to RuntimeError on Windows
        # fp16=True, # Try FP16 instead
        weight_decay=0.01,
        warmup_ratio=0.1
    )
    ...
    if val_results_rows:
        ...
        val_preds_file = 'val_preds_deberta.csv'
        print(f"Validation predictions will be saved to {val_preds_file}")
        save_and_evaluate(final_val_output, val_preds_file, 'DeBERTa', PARAMS, 'dev')
    ...
    if train_results_rows:
        ...
        save_and_evaluate(final_train_output, train_preds_file, 'DeBERTa', PARAMS, 'train')
    
    save_submission(final_output, 'submission_deberta.csv')
    print(f"Saved submission to submission_deberta.csv")
